...
unsigned char* normalize(ENERGY_TYPE *energy, const size_t width, const size_t height, const size_t cpp) {
    ...
    for (size_t i = 0; i < width * height; i++) {
        ...
        for (size_t c = 0; c < cpp; c++) {
            normalized[cpp*i+c] = r;
        }
    }
    printf("%d seems like a normal number\n", max_e);
    return normalized;
}
...
{   
    ...
    for (int row = 0; row < height; row++) {
        ...
        energy[row * width + max_col] = BIG_VALUE;
    }
...
int main(int argc, char *argv[]) {
    ...
    for (size_t i = 0; i < ITERATIONS; i++) {
        ...
        remove_seam(image_energy, image_in, cpp, width, width - i, height, i == ITERATIONS-1);

        double stop = omp_get_wtime();
        if (i % 10 == 0) printf("Seems like time: %f s\n", stop - start);
    }
